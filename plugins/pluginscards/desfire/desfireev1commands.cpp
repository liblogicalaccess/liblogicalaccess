...
namespace logicalaccess
{
	...
	void DESFireEV1Commands::selectApplication(std::shared_ptr<DESFireEV1Location> location)
	{
		...
		getDESFireEV1Chip()->getCrypto()->setKey(location->aid, 0, 0, getDESFireEV1Chip()->getCrypto()->getDefaultKey(location->cryptoMethod));
	}

	void DESFireEV1Commands::createApplication(std::shared_ptr<DESFireEV1Location> location, DESFireKeySettings settings, unsigned char maxNbKeys)
	{

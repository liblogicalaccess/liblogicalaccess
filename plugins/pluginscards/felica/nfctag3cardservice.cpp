...
namespace logicalaccess
{
    ...
    {
        std::shared_ptr<StorageCardService> storage = std::dynamic_pointer_cast<StorageCardService>(getFeliCaChip()->getService(CST_STORAGE));
        ...
        if (data.size() > 0)
            storage->writeData(location, std::shared_ptr<AccessInfo>(), std::shared_ptr<AccessInfo>(), data, CB_AUTOSWITCHAREA);
    }
    ...
    std::shared_ptr<NdefMessage> NFCTag3CardService::readNDEF()
    {
        ...
        if (ndeflen > 0)
        {
            ByteVector data = storage->readData(location, std::shared_ptr<AccessInfo>(), ndeflen, CB_AUTOSWITCHAREA);

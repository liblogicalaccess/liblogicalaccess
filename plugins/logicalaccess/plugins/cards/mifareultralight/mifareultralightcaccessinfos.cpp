...
{
    MifareUltralightAccessInfo::unSerialize(
        node.get_child(MifareUltralightAccessInfo::getDefaultXmlNodeName()));
...
bool MifareUltralightCAccessInfo::operator==(const AccessInfo &ai) const
{
    ...
    const MifareUltralightCAccessInfo *mAi =
        dynamic_cast<const MifareUltralightCAccessInfo *>(&ai);

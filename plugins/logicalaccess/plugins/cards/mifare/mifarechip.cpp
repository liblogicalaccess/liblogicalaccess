...
std::shared_ptr<CardService> MifareChip::getService(CardServiceType serviceType)
{
    ...
    break;
    case CST_STORAGE: { service.reset(new MifareStorageCardService(shared_from_this()));
    }
    break;
    case CST_NFC_TAG: { service.reset(new MifareNFCTagCardService(shared_from_this()));
    }
    break;

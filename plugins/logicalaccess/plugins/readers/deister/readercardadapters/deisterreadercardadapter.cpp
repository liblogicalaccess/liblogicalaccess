...
ByteVector DeisterReaderCardAdapter::prepareDataForDevice(const ByteVector &data)
{
    ...
    for (size_t i = 0; i < data.size(); i++)
    {
        ...
        default: prepareData.push_back(data[i]); break;
        }
    }
    return prepareData;
}
...
ByteVector DeisterReaderCardAdapter::prepareDataFromDevice(const ByteVector &data)
{
    ...
    for (size_t i = 0; i < data.size(); i++)
    {
        if (data[i] == SHFT)
        {
            ...
            switch (data[i])
            {
            case SHFT_SHFT: prepareData.push_back(SHFT); break;

            case SHFT_SOM: prepareData.push_back(SOM); break;

            case SHFT_SOC: prepareData.push_back(SOC); break;

            case SHFT_STOP: prepareData.push_back(STOP); break;

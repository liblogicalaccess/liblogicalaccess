...
DESFireISO7816ResultChecker::DESFireISO7816ResultChecker()
{
    ...
    AddCheck(0x6A, 0x86, "Wrong parameter P1 and/or P2", CardException::WRONG_P1_P2);
    ...
    AddCheck(0x91, 0x0C, "No changes done to backup files. CommitTransaction / "
    ...
    AddCheck(0x91, 0xAF, "Additional data frame is expected to be sent",
             CardException::DEFAULT, false);
    AddCheck(0x91, 0xBE, "Attempt to read/write data from/to beyond the file's/record's "
                         "limits / Attempt to exceed the limits of a value file");
    AddCheck(0x91, 0xC1, "Unrecoverable error within PICC, PICC will be disabled");
    AddCheck(0x91, 0xCA, "Previous Command was not fully completed / Not all Frames were "
                         "requested or provided by the PCD",
             CardException::DEFAULT, false);
    AddCheck(0x91, 0xCD, "PICC was disabled by an unrecoverable error");
    AddCheck(
        0x91, 0xCE,
        "Number of Applications limit reached, no additional CreateApplication possible");
    AddCheck(0x91, 0xDE, "Creation of file/application failed because file/application "
                         "with same number already exists");
    AddCheck(0x91, 0xEE, "Could not complete NV-write operation due to loss of power, "

    ...
    if (d_insertedChip)
    {
        do
        {
            ...
            std::vector<std::shared_ptr<Chip>>::iterator i =
                find_if(chipList.begin(), chipList.end(), Finder(d_insertedChip));
            if (i == chipList.end())
            {
...
unsigned char SCIELReaderUnit::getTimeRemovalFromContextualStack()
{
    ...
    timeRemoval = static_cast<unsigned char>(
        strtoul(BufferHelper::getStdString(ByteVector(r.end() - 2, r.end())).c_str(),
                nullptr, 16));
...
unsigned char SCIELReaderUnit::getADConvertorValue()
{
    ...
    convertorValue = static_cast<unsigned char>(strtoul(
        BufferHelper::getStdString(ByteVector(r.begin() + 2, r.end() - 2)).c_str(),
        nullptr, 16));
...
ChipPowerStatus SCIELReaderUnit::getELAPowerStatus(unsigned char flag)
{
    ...
    switch (flag)
    {
    case 0x00: status = CPS_POWER_HIGH; break;

    case 0x20: status = CPS_POWER_LOW; break;

    default: status = CPS_UNKNOWN; break;
    }

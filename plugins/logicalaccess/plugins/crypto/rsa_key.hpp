 ...
 *
 * A RSAKey is nullable. Before using one of its method, you should ensure that the
 * instance is not null.
 *
 * If the RSAKey is not null, the private key compound can be null, but the public key
 * compound is always defined.
 */
class RSAKey : public AsymmetricKey
{
  public:
     ...
     * \return The RSAKey.
     * \warning If no public compound can be read from data, an InvalidCallException is
     * thrown.
     */
     ...
     * \param data The key data.
     * \return The RSAKey.
     * \warning If no private compound can be read from data, an InvalidCallException is
     * thrown.
     */
     ...
     * \return The RSAKey.
     */
    static RSAKey createFromPEMPublicKey(const ByteVector &data,
                                         PEMPassphraseCallback callback = nullptr,
     ...
     * \return The RSAKey.
     */
    static RSAKey createFromPEMPrivateKey(const ByteVector &data,
                                          PEMPassphraseCallback callback = nullptr,
                                          void *userdata                 = nullptr);
     ...
     * \return The RSAKey.
     */
    static RSAKey createFromPEMPublicKeyFile(const std::string &filename,
                                             PEMPassphraseCallback callback = nullptr,
                                             void *userdata                 = nullptr);
     ...
     * \return The RSAKey.
     */
    static RSAKey createFromPEMPrivateKeyFile(const std::string &filename,
                                              PEMPassphraseCallback callback = nullptr,
                                              void *userdata                 = nullptr);
     ...
     * \param discard_private_compound If true, only the public key part is extracted.
     * Default is false.
     ...
     */
    ByteVector getPEM(bool discard_private_compound  = false,
                      PEMPassphraseCallback callback = nullptr,
     ...
     * \param userdata Some user data to pass to the callback function.
     */
    void writeToPEMKeyFile(const std::string &filename,
                           PEMPassphraseCallback callback = nullptr,
                           void *userdata                 = nullptr) const;
  ...
  private:
     ...
     * \param has_private_key Wether the specified RSA pointer contains a private compound
     * or not.

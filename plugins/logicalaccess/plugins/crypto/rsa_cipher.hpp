...
class RSACipher : public OpenSSLAsymmetricCipher
{
  public:
     ...
     * \param key The key to use. If key is not an instance of RSAKey, the behavior is
     * undefined.
     * \param key_compound The key compound to use.
     * In case of a failure, the call throw an InvalidCallException.
     */
    void cipher(const ByteVector &src, ByteVector &dest, const AsymmetricKey &key,
                KeyCompound key_compound) override;
     ...
     */
    void decipher(const ByteVector &src, ByteVector &dest, const AsymmetricKey &key,
                  KeyCompound key_compound) override;
};
}
}

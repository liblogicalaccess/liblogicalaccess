 ...
 * \brief An EVP PKEY.
 *
 * EVPPKey holds an EVP public or private key. It mainly serves as a wrapper for OpenSSL
 * routines that require a EVP_PKEY.
 *
 * You can create a new EVPPKey from a RSAKey using createFromRSAKey() or do the reverse
 * operation, using rsaKey().
 */
class EVPPKey
{
  public:
     ...
     * \return The RSAKey. If the EVPPKey does not contain any RSAKey, an invalid call
     * exception is thrown.
     */
    RSAKey rsaKey() const;
  ...
  protected:
     ...
     * \param pkey The EVP_PKEY structure pointer. If pkey is NULL, the behavior is
     * undefined.
     * \param has_private_compound true if the EVP_PKEY has a private compound, false
     * otherwise.
     */

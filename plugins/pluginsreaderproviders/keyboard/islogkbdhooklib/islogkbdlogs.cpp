...
namespace islogkbdlib
{
	...
	void KbdLogs::Initialize()
	{
		...
		try
		{
			...
			if (islogkbdlib::KbdSettings::getInstance()->IsLogEnabled)
			{
				...
				std::string fullPath = islogkbdlib::KbdSettings::getInstance()->getDllPath() + "\\" + islogkbdlib::KbdSettings::getInstance()->LogFileName;
	...
	void KbdLogs::LogEvent(const char* Format, ...)
	{
		...
		PrefixSize = sprintf_s(szOutString, sizeof(szOutString), "%02d/%02d/%02d %02d:%02d:%02d.%03d - %d.%d>: ",
					stTime.wYear,stTime.wMonth,stTime.wDay,
					stTime.wHour, stTime.wMinute, stTime.wSecond,
					stTime.wMilliseconds, GetCurrentProcessId(), GetCurrentThreadId());

		va_start(ArgList, Format);

		if (_vsnprintf(&szOutString[PrefixSize], sizeof(szOutString) - PrefixSize, Format, ArgList) < 0)
		{

...
int main(int , char**)
{
	...
	/*	 readerConfig->setReaderProvider(logicalaccess::LibraryManager::getInstance()->getReaderProvider("PCSC"));
		...
		/*readerConfig->setReaderProvider(logicalaccess::LibraryManager::getInstance()->getReaderProvider("Keyboard"));
		readerConfig->setReaderUnit(readerConfig->getReaderProvider()->createReaderUnit());
		boost::shared_ptr<logicalaccess::ReaderUnit> readerUnit = readerConfig->getReaderProvider()->getReaderList()[0];*/
	...
	/*	boost::shared_ptr<logicalaccess::PromagReaderUnitConfiguration> readerUnitConfig(new logicalaccess::PromagReaderUnitConfiguration());
		boost::shared_ptr<logicalaccess::PromagReaderUnit> readerUnit(new logicalaccess::PromagReaderUnit());
		readerUnit->setConfiguration(readerUnitConfig);
		boost::dynamic_pointer_cast<logicalaccess::SerialPortDataTransport>(readerUnit->getDataTransport())
		...
		readerUnit->setConfiguration(readerUnitConfig);
		boost::dynamic_pointer_cast<logicalaccess::SerialPortDataTransport>(readerUnit->getDataTransport())
			->setSerialPort(boost::shared_ptr<logicalaccess::SerialPortXml>(new logicalaccess::SerialPortXml("COM5")));
		boost::dynamic_pointer_cast<logicalaccess::SerialPortDataTransport>(readerUnit->getDataTransport())->setPortBaudRate(9600);

	/*	boost::shared_ptr<logicalaccess::STidSTRReaderProvider> readerProvider =  logicalaccess::STidSTRReaderProvider::getSingletonInstance();
		readerProvider->refreshReaderList();
		logicalaccess::ReaderList readerlist = readerProvider->getReaderList();
		boost::shared_ptr<logicalaccess::ReaderUnit> readerUnit = readerlist[0];*/
            ...
            if (readerUnit->connectToReader())
			{
				...
				while (true)
				{
					...
					if (readerUnit->waitInsertion(500))
					{
						...
						//while (!readerUnit->waitRemoval(500))
						//	std::cout << (double)((std::clock() - start) / (double) CLOCKS_PER_SEC) << "\tWaitRemoval" << std::endl;
						std::cout << (double)((std::clock() - start) / (double) CLOCKS_PER_SEC) << "\tEndWaitRemoval" << std::endl;

					}
					std::cout << "LIST CARDS: " << (int)readerUnit->getReaderChipList().size() << std::endl;
					std::cout << "THERLOL: " << (int)readerUnit->getADConvertorValue() << std::endl;
				}
				readerUnit->disconnectFromReader();
			}           
        }

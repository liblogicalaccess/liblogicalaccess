...
TEST(complete_test, bitset_tests)
{
	...
	bstream.append(0);
	bstream.append(2);
	ASSERT_EQ(402, bstream.getData()[0] * 100 + bstream.getData()[1] * 10 + bstream.getData()[2]);
	bstream.clear();
	...
	bstream.concat(b);
	ASSERT_EQ(bstream.getData()[0] * 1000 + bstream.getData()[1] * 100 + bstream.getData()[2] * 10 + bstream.getData()[3], 2682);
	...
	ASSERT_THROW(bstream.writeAt(0, std::vector<uint8_t>(), 0, 10), std::invalid_argument);

...
TEST(test_format_utils, test_format_RawFormat)
{
    ...
    auto rawData = ByteVector({ 0x89, 0x59, 0x48, 0x48, 0x56, 0x48, 0x35, 0x48, 0x78, 0x99, 0x15, 0x54, 0x36, 0x75, 0x12 });
...
TEST(test_format_utils, test_format_FASCN200BitFormat)
{
    ...
    auto  result = std::vector<unsigned char>({0xD0, 0x43, 0x94, 0x58, 0x21, 0x0C, 0x2C, 0x19, 0xA0, 0x84, 0x6D, 0x83, 0x68, 0x5A, 0x10, 0x82, 0x10, 0x8C, 0xE7, 0x39, 0x84, 0x10, 0x8C, 0xA3, 0xF5 });
   /* 
...
TEST(test_format_utils, test_format_CustomFormat)
{
    ...
    binaryDataField->setValue({ 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xcd, 0xef, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xcd, 0xef });
    ...
    std::shared_ptr<DataRepresentation> littleEdian = std::make_shared<LittleEndianDataRepresentation>();
    ...
    binaryLittleDataField->setValue({ 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xcd, 0xef, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xcd, 0xef });
    ...
    std::shared_ptr<DataRepresentation> nodatarepre = std::make_shared<NoDataRepresentation>();
    ...
    binaryNoDataField->setValue({ 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xcd, 0xef, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xcd, 0xef });
    ...
    auto  result = ByteVector({ });
	ASSERT_EQ(formatBuf, result);
    formatCustom = std::make_shared<CustomFormat>();
    formatCustom->setFieldList(fieldList);
    formatCustom->setLinearData(result);

   auto numberFieldCheck = std::dynamic_pointer_cast<NumberDataField>(formatCustom->getFieldForPosition(0));
   ASSERT_EQ(numberFieldCheck->getValue(), 984654321);

    auto stringFieldCheck = std::dynamic_pointer_cast<StringDataField>(formatCustom->getFieldForPosition(224));
	ASSERT_EQ(stringFieldCheck->getValue(), "Le lamasticot sur kappou ");

    auto binaryFieldCheck = std::dynamic_pointer_cast<BinaryDataField>(formatCustom->getFieldForPosition(489));
    auto value = binaryFieldCheck->getValue();
    auto rvalue = ByteVector({ 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xcd, 0xef, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xcd, 0xef, 0xff, 0xff, 0xff, 0xff});

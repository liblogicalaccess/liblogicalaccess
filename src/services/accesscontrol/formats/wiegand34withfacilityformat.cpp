...
namespace logicalaccess
{
	...
	std::vector<uint8_t> Wiegand34WithFacilityFormat::getLinearDataWithoutParity() const
    {
		BitsetStream data;
    ...
    void Wiegand34WithFacilityFormat::setLinearDataWithoutParity(const std::vector<uint8_t>& data)
    {
        unsigned int pos = 1;
		BitsetStream _data;
		_data.concat(data);
    ...
    size_t Wiegand34WithFacilityFormat::getFormatLinearData(std::vector<uint8_t>& data) const
    {
    ...
    bool Wiegand34WithFacilityFormat::checkSkeleton(std::shared_ptr<Format> format) const
    {
        ...
        if (format)
        {
            std::shared_ptr<Wiegand34WithFacilityFormat> pFormat = std::dynamic_pointer_cast<Wiegand34WithFacilityFormat>(format);
            if (pFormat)
            {
                ret = (

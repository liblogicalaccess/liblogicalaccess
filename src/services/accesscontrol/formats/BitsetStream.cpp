...
{
	BitsetStream::BitsetStream() : _pos(0)
	{
	}

        BitsetStream::BitsetStream(unsigned char data, unsigned int byteSize) : _pos(0)
        {
	...
	BitsetStream::BitsetStream(unsigned long long bitsSize) : _pos(0)
	{
	...
	void BitsetStream::append(unsigned char data, unsigned int readPosStart)
	{
        ...
        void BitsetStream::append(unsigned char data, unsigned int readPosStart, unsigned int readLength)
        {
            ...
            stream[block] |= (data >> (8 - (readPosStart + readLength)) << ((8 - (readPosStart + readLength)) + readPosStart) & 0xFF) >> offset;
            unsigned char newData = data >> (8 - (readPosStart + readLength)) << (8 - (readPosStart + readLength) + readPosStart) << (8 - offset);
	...
	void BitsetStream::concat(const ByteVector& data, unsigned int readPosStart)
	{
        ...
        void BitsetStream::concat(const ByteVector& data, unsigned int readPosStart, unsigned int readLength)
        {
            if (data.empty())
            ...
            while (readBits < readLength)
            {
                ...
                {
                    unsigned char length = (readLength - readBits) < 8 ? (readLength - readBits) : 8;
	...
	void BitsetStream::writeAt(unsigned int pos, unsigned char data, unsigned int readPosStart, unsigned int readLength)
	{
		...
		if ((8 - offset) >= readLength)
		{
			...
			stream[block] = (((stream[block] << offset << readLength & 0xFF) >> readLength) | (data << readPosStart & 0xFF)) >> offset | save;
		}
		else
		{
			...
			stream[block] |= ((data >> (8 - (readPosStart + readLength))) << ((8 - (readPosStart + readLength)) + readPosStart) & 0xFF) << (8 - offset);
		}
	}

        void BitsetStream::writeAt(unsigned int pos, ByteVector const& data, unsigned int readPosStart, unsigned int readLength)
        {
            if (data.empty())
                throw std::invalid_argument("Data vector is empty.");
            ...
            while (readBits < readLength)
            {
                if (offset + (readLength - readBits) > 8)
                {
                    const unsigned char length = (readLength - readBits) < 8 ? (readLength - readBits) : 8;
                    const unsigned char newBlock = (data[i] << offset & 0xFF) + (data[i + 1] >> (8 - offset));
	...
	void BitsetStream::insert(unsigned int pos, unsigned char data, unsigned int readPosStart, unsigned int readLength)
	{
		...
		else
		{
			...
			if (_pos % 8 > readLength)
			{
				...
				stream[block] = ((stream[block] << (pos % 8) & 0xFF) >> (pos % 8) >> readLength & 0xFF) | save;
			}
	...
	void BitsetStream::insert(unsigned int pos, ByteVector const& data, unsigned int readPosStart, unsigned int readLength)
	{
		if (data.empty())
			throw std::invalid_argument("Data vector is empty.");

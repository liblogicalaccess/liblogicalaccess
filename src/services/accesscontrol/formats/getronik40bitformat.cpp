...
namespace logicalaccess
{
    ...
    void Getronik40BitFormat::setFormatLinearData(const ByteVector& /*data*/, size_t* /*indexByte*/)
    {
        //DOES NOTHING
    }
    ...
    void Getronik40BitFormat::setLinearData(const ByteVector& data)
    {
        ...
        if (_data.getByteSize() != 0)
        {
            ...
            if (fixedValue.getData()[0] != 0x2E)
            {
                ...
                sprintf(exceptionmsg, "Getronik 40-Bit: fixed left value doesn't match (%x != %x).", fixedValue.getData()[0], 0x2E);
            ...
            {
                char exceptionmsg[256];
                sprintf(exceptionmsg, "Getronik 40-Bit: fixed right value doesn't match (%x != %x).", fixedValue.getData()[0], 0x01);
                THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException, exceptionmsg);
            }
            pos = 8;
